...
# Set environment variable before importing InvocationBackgroundTasks
...
class TestBackgroundClass:
    ...
    async def test_start_cache_warming_task(self, background_tasks, caplog):
        with patch.object(
            background_tasks, "warm_all_user_caches", new_callable=AsyncMock
        ) as mock_warm:
    ...
    async def test_warm_all_user_caches(self, background_tasks, mock_redis):
        ...
        with patch.object(
            background_tasks, "warm_user_cache", new_callable=AsyncMock
        ) as mock_warm:
            mock_warm.return_value = None
    ...
    async def test_decrypt_api_token_success(self, background_tasks):
        with patch("app.orchestration.invocation_tasks.Fernet") as mock_fernet:
            ...
            mock_instance.decrypt.return_value = json.dumps(
    ...
    async def test_fetch_workflows_safely_success(self, background_tasks):
        ...
        result = await background_tasks.fetch_workflows_safely(
            mock_workflow_manager, fetch_details=True
        )
        assert len(result) == 2
...
class TestInvocationCache:
    ...
    @pytest.mark.asyncio
    async def test_get_invocation_workflow_mapping_hit(
        ...
        mock_redis.hgetall.return_value = {
            b"inv1": json.dumps(
                {"workflow_name": "wf1", "workflow_id": "wf_id1"}
            ).encode()
        }
        ...
        mock_redis.hgetall.assert_called_once_with(
            f"invocation_workflow_map:{username}"
        )

    @pytest.mark.asyncio
    async def test_get_invocation_workflow_mapping_miss(
        self, cache_instance, mock_redis
    ):
    ...
    @pytest.mark.asyncio
    async def test_set_invocation_workflow_mapping_success(
        self, cache_instance, mock_redis
    ):
        ...
        mock_pipeline.expire.assert_called_once_with(
            f"invocation_workflow_map:{username}", ttl
        )
    ...
    async def test_get_response_cache_hit(self, cache_instance, mock_redis):
        ...
        result = await cache_instance.get_response_cache(
    ...
    async def test_set_response_cache_success(self, cache_instance, mock_redis):
        ...
        await cache_instance.set_response_cache(
            username, response, workflow_id, history_id, ttl
        )
    ...
    async def test_get_invocations_cache_hit(self, cache_instance, mock_redis):
        ...
        assert result == expected_data
        mock_redis.get.assert_called_once_with(
            f"invocations_raw:{username}:workflow_id:wf1"
        )
    ...
    async def test_add_to_deleted_invocations_success(self, cache_instance, mock_redis):
        ...
        mock_redis.sadd.assert_called_once_with(
            f"deleted_invocations:{username}", *invocation_ids
        )
    ...
    async def test_is_duplicate_request_false(self, cache_instance, mock_redis):
        ...
        mock_redis.exists.assert_called_once_with(
            f"request_dedup:{username}:{request_hash}"
        )
        mock_redis.setex.assert_called_once_with(
            f"request_dedup:{username}:{request_hash}", 3, "1"
        )
    ...
    @pytest.mark.asyncio
    async def test_is_duplicate_workflow_request_false(
        self, cache_instance, mock_redis
    ):
        ...
        result = await cache_instance.is_duplicate_workflow_request(
            username, request_hash
        )

        assert result is False
        mock_redis.exists.assert_called_once_with(
            f"workflow_request_dedup:{username}:{request_hash}"
        )
        mock_redis.setex.assert_called_once_with(
            f"workflow_request_dedup:{username}:{request_hash}",
            TTLiveConfig.DUPLICATE_CHECK.value,
            "1",
        )
    ...
    async def test_get_invocation_result_hit(self, cache_instance, mock_redis):
        ...
        assert result == expected_data
        mock_redis.get.assert_called_once_with(
            f"invocation_result:{username}:{invocation_id}"
        )
    ...
    async def test_set_invocation_state_invalid_state(self, cache_instance, mock_redis):
        ...
        with patch.object(cache_instance, "log") as mock_log:
            ...
            mock_log.error.assert_called_once_with(
                "Error setting invocation state: Invalid state: Invalid"
            )
    ...
    async def test_get_invocation_state_hit(self, cache_instance, mock_redis):
        ...
        mock_redis.hget.assert_called_once_with(
            f"invocation_states:{username}", invocation_id
        )
    ...
    async def test_add_deleted_workflows_success(self, cache_instance, mock_redis):
        ...
        mock_pipeline.sadd.assert_called_once_with(
            f"deleted_workflows:{username}", *workflow_ids
        )
        mock_pipeline.expire.assert_called_once_with(
            f"deleted_workflows:{username}", TTLiveConfig.WORKFLOW_CACHE.value
        )
        mock_pipeline.execute.assert_called_once()
