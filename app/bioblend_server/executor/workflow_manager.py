...
class WorkflowManager:
    ...
    def __init__(self, galaxy_client: GalaxyClient):
        ...
        self.gi_admin = (
            self.galaxy_client.gi_admin
        )  # For administrative functionalitites like toolshed instantiation and tool installing
    ...
    async def upload_workflow(
        self, path: str, ws_manager: SocketManager, tracker_id: str
    ) -> Workflow:
        ...
        try:
            ...
            await ws_manager.broadcast(
                ...
                data={
                    ...
                    "payload": {
                        "message": f"Error installing missing tools in the uploaded workflow: {e}"
                    },
                },
                tracker_id=tracker_id,
            )
            self.log.error(
                f"Error installing missing tools in the uploaded workflow: {e} traceback:{traceback.format_exc()}"
            )
        ...
        workflow = await asyncio.to_thread(
            self.gi_object.workflows.import_new, src=workflow_json, publish=False
        )
    ...
    def get_worlflow_by_name(
        self, name: str, score_cutoff: int = 70
    ) -> Workflow | None:
    ...
    def tool_exists(self, step: dict) -> bool:
        ...
        if step_repo:
            ...
            if tool_repo.get("changeset_revision") != step_repo.get(
                "changeset_revision"
            ):
                return False
    ...
    async def tool_check_install(
        self, step: dict, ws_manager: SocketManager, tracker_id: str
    ):
        ...
        if not self.tool_exists(step):
            ...
            try:
                ...
                if isinstance(dict, install_result):
                    self.log.info(
                        f"status: {install_result.get('status')}, message: {install_result.get('message')}"
                    )
                    await ws_manager.broadcast(
                        event=SocketMessageEvent.workflow_upload,
                        data={
                    ...
                    for repo_info in install_result:
                        ...
                        error_msg = (
                            repo_info.get("error_message", "None")
                            if status != "installed"
                            else "None"
                        )
                ...
                self.log.error(
                    f"Failed to install tool '{toolshed_info['name']}': {str(e)}  traceback:{traceback.format_exc()}"
                )
                raise
        else:
    ...
    async def validate_input_to_tool(self, mapped_input: dict, workflow: dict) -> dict:
        ...
        for step in steps.values():
            ...
            for param_name, conn_info in input_steps.items():
                ...
                input_connections_map.setdefault(source_step, []).append(
                    (tool_id, param_name)
                )
                input_tools.add(tool_id)

        self.log.info(
            f"Tools that directly take input from workflow inputs are: {input_tools}"
        )
        ...
        for tool_id in input_tools:
            try:
                xml = await self.tool_manager.get_tool_xml(tool_id)
                if xml:
                ...
                self.log.error(f"Unexpected error for tool {tool_id}: {e}")
        ...
        for input_id, input_details in mapped_input.items():
            ...
            any_connection_failed = False

            connections = input_connections_map.get(str(input_id), [])

            if not connections:
                final_validation["status"] = "not_validated"
                input_details["validation"] = final_validation
                continue

            for tool_id, param_name in connections:
                root = tool_xml_cache.get(tool_id)
                if root is None:
                    any_connection_failed = True
                    final_validation["message"] = (
                        f"Tool XML for '{tool_id}' could not be loaded."
                    )
                    continue

                actual_param_name = param_name.split("|")[-1]
                param_node = root.find(f".//param[@name='{actual_param_name}']")
                if param_node is not None:
                    ...
                    if not final_validation["rules"]:
                        final_validation["rules"] = validation_info
                    else:
                        existing_formats = set(
                            final_validation["rules"].get("format", "".split(","))
                        )
                        new_formats = set(validation_info.get("format", "").split(","))
                        final_validation["rules"]["format"] = ",".join(
                            sorted(list(existing_formats.intersection(new_formats)))
                        )

                else:
                    any_connection_failed = True
                    final_validation["message"] = (
                        f"Could not find parameter '{param_name}' in tool '{tool_id}'. The workflow may be misconfigured."
                    )
                    self.log.warning(final_validation["message"])

            if any_connection_failed:
                final_validation["status"] = "failed"
                final_validation["rules"] = {}

            input_details["validation"] = final_validation

        return mapped_input

    async def build_input(self, workflow: Workflow, history: History) -> str:
        ...
        mapped_input = self._map_inputs_to_steps(
            name=workflow["name"], inputs=workflow["inputs"], steps=workflow["steps"]
        )

        # builds a html form from the mapped input payload and validate it
        mapped_input = await self.validate_input_to_tool(mapped_input, workflow)
        form_generator = WorkflowFormGenerator(
            mapped_workflow=mapped_input,
    ...
    async def track_invocation(
        ...
        while True:
            inv = await asyncio.to_thread(
                self.gi_object.gi.invocations.show_invocation,
            ...
            step_jobs = await asyncio.to_thread(
                self.gi_object.gi.invocations.get_invocation_step_jobs_summary,
                invocation_id=invocation.id,
            )
            ...
            for step in step_jobs:
                ...
                if current_state != prev and current_state in (
                    "running",
                    "ok",
                    "error",
                ):
                    self.log.info(
                        f"Step {step_index} with id {step_id} transitioned to {current_state}"
                    )
                ...
                if current_state == "error":
                    self.log.error(
                        f"Step {step_index} with id {step_id} failed; cancelling invocation"
                    )
            ...
            if now > deadline:
                self.log.error(
                    f"Invocation timed out after {int(now - start_time)} seconds."
                )
                ws_data = {
                    "type": SocketMessageType.INVOCATION_FAILURE,
    ...
    def invoke_workflow(
        self, inputs: dict, workflow: Workflow, history: History
    ) -> Invocation:
        """
        ...
        invoke = self.gi_object.gi.workflows.invoke_workflow(
    ...
    def _make_result(
        self, invocation: Invocation, outputs: list[Union[Dataset, DatasetCollection]]
    ):
        """Result report of the workflow invocation"""
        invocation_id = invocation.id
        invocation_report = self.gi_object.gi.invocations.get_invocation_report(
            invocation.id
        )
        final_outputs = []
    ...
    async def run_track_workflow(
        ...
        outputs = await self.track_invocation(
            invocation, tracker_id, ws_manager
        )  # can set base time extenstion and max_wait here
        # prepare workflow invocation results
        invocation_id, invocation_report, intermediate_outputs, final_outputs = (
            self._make_result(invocation, outputs)
        )
