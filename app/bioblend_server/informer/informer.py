...
from app.bioblend_server.informer.prompts import (
    RETRIEVE_PROMPT,
    SELECTION_PROMPT,
    INVOCATION_PROMPT,
)
...
class GalaxyInformer:
    ...
    def __init__(self, galaxy_client: GalaxyClient, entity_type: str):
        ...
        self.logger.info(
            f'Initializing the galaxy informer for entity type: {entity_type} for user {self.user_info["id"]}'
        )
    ...
    def _get_datasets(self) -> list[dict]:
        ...
        try:
            ...
            for library in libraries:
                library_details = self.gi_user.libraries.show_library(
                    library["id"], contents=True
                )
    ...
    def _get_tools(self) -> list[dict]:
        ...
        for tool in self.gi_user.tools.get_tools():
            ...
            if tool_type == "data manager":
                ...
                tool_info[
                    "content"
                ] += f'This Galaxy tool named  {tool_info["name"]} is a regular galaxy tool'
    ...
    def _fuzzy_search(
        self, query: str, entities: list[dict], config: list, threshold=85
    ) -> list:
        ...
        results = process.extract(
            query, priority_candidates, scorer=fuzz.WRatio, limit=10
        )
        ...
        if priority_matches:
            self.logger.info(
                f"Found {len(priority_matches)} matches in priority fields for search query: {query}"
            )
        ...
        self.logger.info(
            f"No matches found in the priority fields for search query: {query}, searching in all fields as a fallback"
        )
        ...
        results = process.extract(
            query, fallback_candidates, scorer=fuzz.WRatio, limit=10
        )
        ...
        if fallback_matches:
            self.logger.info(
                f"Found {len(fallback_matches)} matches in fallback fields for search query: {query}"
            )
            return sorted(fallback_matches, key=lambda x: x[1], reverse=True)[:5]

        self.logger.info(
            f"No matches found in either priority or fallback fields for search query: {query}"
        )
        return []  # Always return a list, even if empty

    async def refresh_and_cache_entities(self) -> list[dict]:
        """
        Loads global entity data from pre-processed files, enriches it with instance availability,
        improves content formatting for better embeddings, then caches it in Redis and Qdrant.
        """
    ...
    def get_entity_details(self, entity_id: str) -> dict:
        ...
        try:
            self.logger.info(
                f"Fetching details for {self.entity_type} with ID: {entity_id}"
            )
            return detail_methods[self.entity_type](entity_id)
        except Exception as e:
            self.logger.error(
                f"Could not retrieve details for {self.entity_type}:{entity_id}: {e}"
            )
        ...
        try:
            ...
            if is_close_to_invocation(query=query):
                ...
                if response == "No matches":
                    ...
                    invocation_detail = self.gi_user.invocations.show_invocation(
                        invocation_id=response
                    )
                    invocation_report = self.gi_user.invocations.get_invocation_report(
                        invocation_id=response
                    )
                    ...
                    invocation_input_step_parameters = invocation_detail.get(
                        "input_step_parameters", None
                    )
                    invocation_outputs = [
                        key
                        for key, value in invocation_detail.get("outputs", None).items()
                    ]
                    invocation_output_collection = [
                        key
                        for key, value in invocation_detail.get(
    ...
    async def generate_final_response(
        self, query: str, retrieved_content: dict
    ) -> dict:
        """
        ...
        prompt = RETRIEVE_PROMPT.format(
            query=query, retrieved_content=retrieved_content
        )
    ...
    async def get_entity_info(self, search_query: str, entity_id: str = None) -> dict:
        ...
        # This search_entities call now correctly handles caching/refreshing internally
        found_entities = await self.search_entities(query=search_query)

        if not found_entities:
            self.logger.warning(f"Semantic search for '{search_query}' yielded no specific results. Falling back to listing available items.")
            local_entities = self.get_all_entities()
            response_content = {
                "message": f"I couldn't find a specific match for your query, but here are some of the {self.entity_type}s available in your instance:",
                "available_items": [entity.get('name', 'Unknown') for entity in local_entities[:10]]
            }
            return await self.generate_final_response(search_query, response_content)

        self.logger.info(f'Found entities via semantic search: {found_entities}')
        
        response_details = {}

        # Check for invocation details only if it's a workflow and we found a specific one
        if self.entity_type == 'workflow' and '0' in found_entities:
            workflow_id_for_invocation = found_entities['0'].get(id_field)
            if workflow_id_for_invocation:
                invocation_info = await self.get_invocation_details(query=search_query, workflow_id=workflow_id_for_invocation)
                response_details['invocation_details'] = invocation_info or "No matching invocation details found for this workflow."

        # Collate details for all found entities
        for i, item_stub in enumerate(found_entities.values()):
            item_id = item_stub.get(id_field) or item_stub.get("workflow_repository") # Handle workflow ID
            if item_id:
                # We need to get the full details from the cached master list, not the API
                master_list = await self.get_cached_or_fresh_entities()
                details = next((e for e in master_list if (e.get(id_field) or e.get("workflow_repository")) == item_id), None)
                
                if details:
                    response_details[f'found_{self.entity_type}_{i}'] = details
