...
class WorkflowGitubScraperUrl(Enum):
    GITHUB_SCRAPE_URL = (
        "https://api.github.com/repos/galaxyproject/iwc/contents/workflows"
    )
...
class LLMResponse:
    ...
    def embedding_size(self) -> int:
        ...
        provider = EmbeddingProvider(
            os.getenv("CURRENT_EMBEDDER", EmbeddingProvider.GEMINI.value)
        ).value
    ...
    def llm(self) -> LLMProvider:
        provider = TextProvider(
            os.getenv("CURRENT_LLM", TextProvider.GEMINI.value)
        ).value
        selected_config = LLMModelConfig(self.config["providers"][provider])
    ...
    async def get_embeddings(self, input):
        ...
        if embed.shape[-1] != self.embedding_size:
            raise ValueError(
                f"Expected embedding dimension {self.embedding_size}, got {embed.shape[-1]}"
            )
